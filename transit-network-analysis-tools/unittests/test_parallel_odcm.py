...
class TestParallelODCM(unittest.TestCase):
    ...
    def setUpClass(self):  # pylint: disable=bad-classmethod-argument
        ...
        self.parallel_od_class_args = {
            ...
            "time_window_start_time": "10:00",
            "time_window_end_day": "Wednesday",
            "time_window_end_time": "10:02",
            "time_increment": 1,
            "max_processes": 4,
            "time_units": "Minutes",
            "cutoff": 30,
    ...
    def test_ODCostMatrix_select_inputs(self):
        ...
        dest_criteria = [4, 8]  # Encompasses five destinations, two of which have > 10 jobs
    ...
    def test_ParallelODCalculator_solve_od_in_parallel_cam_weight(self):
        ...
        expected_fields = ["TotalDests", "PercDests"] + \
                          [f"DsAL{p}Perc" for p in range(10, 100, 10)] + \
                          [f"PsAL{p}Perc" for p in range(10, 100, 10)]
        self.assertTrue(
            set(expected_fields).issubset({f.name for f in arcpy.ListFields(test_origins)}),
        ...
        max_dests = 0
        for row in arcpy.da.SearchCursor(test_origins, expected_fields):
            for val in row:
    ...
    def test_ParallelODCalculator_solve_od_in_parallel_ctts(self):
        ...
        expected_columns = ["OriginOID", "DestinationOID", "count", "min", "max", "mean"]
        self.assertEqual(expected_columns, df.columns.tolist(), "Incorrect columns in CSV")
